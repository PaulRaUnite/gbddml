...
-------------
	...
	const char* GBdd::version() (m�thode statique)

	1.1 
	Compatibilit� avec g++ > 4
...
- les op�rateurs == et != sont definis et rendent un int (0 ou 1).
...
10) Liste Cha�n�e de bdds :
---------------------------
	Attention tr�s sommaire : la liste vide est cod�e par NULL,
	il n'y a aucun partage ni gestion m�moire =>
	- ne manipuler QUE les t�tes de liste,
	- pensez � d�truire les listes inutiles (N.B. la destruction est 
	r�cursive)
...
int GBdd::garbage_collect() -> renvoie le %age de noeuds lib�r�s
...
public :
	...
	*/
	...
	/*
	Le nombre de noeuds r�ellement utilis�s
	*/
...
/*---------------------------------------------------------------
   Version 1.1 => � partir de g++ > 4
   les fonctions friend doivent �tre re-d�clar�es en dehors
   de la classe pour �tre r�f�renc�es comme export� dans le .o
----------------------------------------------------------------*/
...
int compare(const GBdd& t1, const GBdd& t2); // retourne 0 SSI �galit�
//Comparaisons
...
public:
   ...
   GBddList* tail(){ return nxt; }
	//Renvoie le suivant et d�truit la t�te
